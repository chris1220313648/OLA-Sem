...
from utils.inference_schedule import build_joint_denoising_timesteps
...
class Motus(nn.Module):
    ...
    def inference_step(
        ...
        future_video_denoise_fraction: float = 1.0,
        ...
        if self.config.flow_source_mode == "history":
            ...
            action_latent = torch.randn(
                action_shape, device=self.device, dtype=self.dtype
            )
        ...
        # 3. Action fully denoises; video may stop at an intermediate noise level.
        video_timesteps, action_timesteps = build_joint_denoising_timesteps(
            num_inference_steps,
        ...
        for i in range(num_inference_steps):
            video_t = video_timesteps[i]
            video_dt = video_timesteps[i + 1] - video_t
            action_t = action_timesteps[i]
            action_dt = action_timesteps[i + 1] - action_t
            video_t_scaled = (video_t * 1000).expand(B).to(self.dtype)
            action_t_scaled = (action_t * 1000).expand(B).to(self.dtype)
